...
impl HtmlContext {
    ...
    pub fn as_string(&self) -> Option<String> {
        let lang = self
            ...
            .borrow()
            .as_ref()
            .map(|val| format!("lang=\"{}\"", leptos::leptos_dom::ssr::escape_attr(&val.get())));
        let dir = self
            .dir
            .borrow()
            .as_ref()
            .map(|val| format!("dir=\"{}\"", leptos::leptos_dom::ssr::escape_attr(&val.get())));
        let class = self
            .class
            .borrow()
            .as_ref()
            .map(|val| format!("class=\"{}\"", leptos::leptos_dom::ssr::escape_attr(&val.get())));
        let attributes = self.attributes.borrow().as_ref().map(|val| {
            val.with(|val| {
                val.into_iter()
                    .map(|(n, v)| format!("{}=\"{}\"", n, leptos::leptos_dom::ssr::escape_attr(&v.get())))
                    .collect::<Vec<_>>()
                    .join(" ")
            })
        });
